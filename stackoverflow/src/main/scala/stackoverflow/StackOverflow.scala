...
object StackOverflow extends StackOverflow:
  ...
  def main(args: Array[String]): Unit =
    ...
    val grouped = groupedPostings(raw)  //RDD[(QID, Iterable[(Question, Answer)])]
    val scored  = scoredPostings(grouped) //RDD[(Question, HighScore)]
    val vectors = vectorPostings(scored) //RDD[(LangIndex, HighScore)]
...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] =
    ...
    val q_as_pair = grouped.flatMap(g => g._2).groupByKey()
    val q_highscore_pair = q_as_pair.mapValues(ans => answerHighScore(ans.toArray))
    q_highscore_pair
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] =
    ...
    val newMeansMap: scala.collection.Map[(Int, Int), (Int, Int)] = {
        val currentMean_vectors = vectors.map(vec => (findClosest(vec, means), vec)).groupByKey() //RDD[MEAN,  Iterable[(Int, Int)]]
        val currentMean_newMean = currentMean_vectors.mapValues(vecs => averageVectors(vecs))  //RDD[(Int, Int), (Int, Int)]
        currentMean_newMean.collect().toMap
    }
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] =
    ...
    val median = closestGrouped.mapValues { vs =>
      val mostLangIdx_count = vs.groupBy(_._1).view.mapValues(_.size).maxBy(_._2)
      val (mostLangIdx, mostLangCount) = mostLangIdx_count
      val langLabel: String = langs(mostLangIdx / langSpread)    // most common language in the cluster
      val langPercent: Double =  (mostLangCount / vs.size) * 100 // percent of the questions in the most common language
